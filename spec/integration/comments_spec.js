...
describe("routes : comments", () => {
  ...
  describe("signed in user performing CRUD actions for Comment", () => {
    ...
    describe("POST /topics/:topicId/posts/:postId/comments/:id/destroy", () => {
      ...
      it('should not delete the comment if the associated user does not match the comment\'s userId', done => {
        Comment.all()
        .then(comments => {
          const commentsBeforeDelete = comments.length;
          ...
          .then(user => {
            ...
            request.post(`${base}${this.topic.id}/posts/${this.post.id}/comment/${this.comment.id}/destroy`,
              (err, res, body) => {
                expect(comments.length).toBe(commentsBeforeDelete);
                done();
              });
          });
        })
  ...
  describe('admin user performing CRUD actions on comments', () => {
    describe('POST /topics/:topicId/posts/:id/comments/:commentId/destroy', () => {
      it('should allow the admin to delete other users\'s comments', done => {
        User.create({
          email: 'andy@fake.net',
          password: '123456'
        })
          ...
          .then(comments => {
            ...
            request.post(`${base}${this.topic.id}/posts/${this.post.id}/comments/${this.comment.id}/destroy`,
              (err, res, body) => {
                Comment.all()
                .then(comments => {
                  expect(comments.length).toBe(commentsBeforeDelete - 1);
                  done();
                })
              })
          })
